...
WHERE
  ...
  AND NOT (
    ...
    AND (grcts.a_leads_recent IS NULL OR ARRAY_LENGTH(grcts.a_leads_recent) = 0)
    AND (
    ...
    AND (grcts.avg_session_duration IS NULL OR grcts.avg_session_duration = 0)
    AND (grcts.a_url IS NULL OR ARRAY_LENGTH(grcts.a_url) = 0)
    AND (
      grcts.a_emailsubject IS NULL OR ARRAY_LENGTH(grcts.a_emailsubject) = 0))
