...
class CustomTabTransformer(nn.Module):
    def __init__(self, categories=categories, num_continuous=1,output_dim=512):#token_vocab_size=25,, dropout=0.1 token_emb_dim=128, gru_hidden_dim=128,
       ...
       # Projection layers to same dimension
        self.tab_proj = nn.Linear(512, 512)   #should BN or add dropout? 128=dim_out
        self.tab_relu = nn.ReLU()
        self.tab_norm = nn.LayerNorm(512)
...
def tokenize_into_token_chunks(text, tokenizer, max_len, max_chunks, max_total_tokens, add_special_tokens=False):
    """
    Example implementation: splits token ids into sliding/chunked arrays.
    Returns list of dicts each with 'input_ids' and 'attention_mask'.
    """
    ...
    # If text empty, produce at least one empty chunk
    if len(chunks) == 0:
...
class MultiTextEncoder(nn.Module):
    ...
    def _encode_field_batch(self, texts, field_name):
        ...
        returns:
          chunk_embeddings: Tensor [B, max_chunks, H]
          chunk_mask: Tensor [B, max_chunks] (1 where a real chunk exists)
        """
...
class FusionModel(nn.Module):
    def __init__(self, tab_backbone: nn.Module, text_encoder: MultiTextEncoder, fusion_hidden: int = 256, out_dim: int = 1):
        ...
        self.residual_tab = ResidualBlock(dim=512)
        self.residual_text = ResidualBlock(dim=512)
        

        self.g = GatingLayer(1024)
...
def concat_first_n(x,n=30 ,fixed_width=False):
    ...
    if fixed_width:
        ...
        s = " ".join(((s or "")[:10].ljust(n) for s in x))
        return s.rstrip()[:300]
    else:
...
def safe_join(x):
    ...
    if isinstance(x, list):  
        ...
        x = concat_first_n(x,n=25, fixed_width=False)
        return x[:400]              
        ...
        if cleaned.startswith("[") and cleaned.endswith("]"):
            try:
                ...
                if isinstance(parsed, list):
                    parsed  = concat_first_n(parsed,n=25, fixed_width=False)
                    return parsed[:400]                            
                    #return " ".join(str(item).strip() for item in parsed if item).strip()[0:300]
            except Exception:
                pass
               
        return cleaned[0:400]

    return str(x).strip()[0:400]
